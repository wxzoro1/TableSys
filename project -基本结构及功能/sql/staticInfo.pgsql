...
CREATE TABLE IF NOT EXISTS coursepre  (
    sn            INTEGER,          --序号
    institute     TEXT,             --学院(已选)
    term          TEXT,             --学期(已选)
    courseno      INTEGER,          --课程号(待填写)
    courseno2     INTEGER,          --课序号(待填写)
    cname         TEXT,             --课程名(待填写)
    credit        INTEGER,          --学分(待填写)
    property      TEXT,             --课程属性(待填写)
    PRIMARY KEY(courseno,courseno2)
);
-- 给sn创建一个自增序号
...
CREATE TABLE IF NOT EXISTS courseplan (
    ...
    term          TEXT,              --学期(已选)
    major         TEXT,              --专业(已选)
    courseno      INTEGER,           --课程号(待绑定)
    courseno2      INTEGER,          --课序号(待绑定)
    PRIMARY KEY(sn)
);
-- 给sn创建一个自增序号
CREATE SEQUENCE seq_courseplan_sn 
    START 1 INCREMENT 1 OWNED BY courseplan.sn;
...
CREATE TABLE IF NOT EXISTS ctable(
    ...
    term          TEXT,              --学期(已选)
    major         TEXT,              --专业(已选)
    courseno      INTEGER,           --课程号(已选)
    courseno2     INTEGER,           --课序号(已选)
    cname         TEXT,              --课程名(已选)
    credit        INTEGER,           --学分(已选)
    property      TEXT,              --课程属性(已选)
...
ALTER TABLE courseplan
    ADD CONSTRAINT cp_cc_fk FOREIGN KEY (courseno,courseno2) REFERENCES coursepre(courseno,courseno2);
ALTER TABLE ctable
    ADD CONSTRAINT ct_cc_fk FOREIGN KEY (courseno,courseno2) REFERENCES coursepre(courseno,courseno2);
